...
为了更清晰地展示 ROCm-JIT-Agent 在实际项目中的运转时机，以下是完整的时序数据流说明：

1. **触发拦截 (Interception)**：
   * **时机**：用户代码（如测试脚本或训练主循环）**第一次**调用被 `@rocm_jit_agent.optimize` 装饰的函数时。
   * **动作**：系统挂起原生函数的执行，抓取函数的 AST 源码，并获取传入张量的动态信息（如 `shape`、`dtype`、`stride`）。

2. **模型推理 (Model Inference)**：
   * **时机**：拦截成功后，引擎提取完上下文立刻触发。
   * **动作**：将 AST 与张量签名组装成硬核 Prompt，唤醒本地 4B 专精代码模型（或连接云端网关），执行前向推理，首次吐出底层 HIP C++ 或 Triton 算子代码。

3. **效果验证 (Effect Validation)**：
   * **时机**：模型吐出算子代码并完成 JIT 编译后。
   * **动作**：在隔离的 `subprocess` 沙盒中给算子喂入随机测试张量，并与 PyTorch 原生 Eager 模式的标答结果计算 MSE。如果精度不达标或编译报错（Segfault/语法错误），系统会携带错误日志重新**触发模型推理**进行闭环修复。

4. **性能榨取 (Performance Profiling)**：
   * **时机**：正确性验证通过（MSE=0.0）后，正式替换前。
   * **动作**：调用 `rocprofv3` 对新算子进行跑分。结合硬件 Roofline 理论极限带宽，如果发现性能不佳（如 VGPR 溢出、指令流水线气泡），系统会将硬件日志翻译为反馈提示词，再次**触发模型推理**进行汇编级的靶向优化。

5. **测试运行与永久缓存 (Test Running & Caching)**：
   * **时机**：当算子性能达标或达到最大重试次数（如 Iter 4/4）后，以及未来的每一次调用。
   * **动作**：优化的动态链接库（`.so`）或 Triton 字节码会被硬编码落盘到 `~/.rocm_jit_agent_cache`。当前被拦截的这次调用以及**后续所有的测试运行（如接下来的 Benchmark 循环或训练 Epoch）**，都会跳过大模型，以 O(1) 零延迟直接命中缓存执行。

---

## 4. 设计哲学：为什么不用云端，而是死磕本地 4B 训练？
